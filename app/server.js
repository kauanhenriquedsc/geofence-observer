...
const ALERT_PHONE = '+5515996189346'; 

app.use(express.json());geofencei
app.use(cors());
...
// buffer unit is km
GEOFENCE_COVERAGE = 0.02;
const route = turf.lineString(routeCoords);
const buffer = turf.buffer(route, GEOFENCE_COVERAGE, { units: 'kilometers' }); 
// let clienteRelativePosition = [];
...
app.post('/check-geofence', async (req, res) => {
    ...
    // point in polygon - search if Point (Geometry object)
    // is inside a linestring buffer like a linear geofence
    // this approach using simple idea for verify if point inside a polygon.
    // The algorithm make a line from the point until ntersect in geofence line
    // ray casting algorithm for verify if a geopoint (geolocate) is inside or not 
    // of geofence
    const inside = turf.booleanPointInPolygon(pt, buffer);
    if (lastInside && !inside) {
        sendSMS(
            ALERT_PHONE,
            `⚠️ Alerta: O dispositivo saiu da geocerca às ${new Date().toLocaleString()}.`
        );
        console.log(`[ALERTA] Veículo saiu da geocerca! ${new Date().toLocaleString()}`);
    }
    // Atualiza o estado para a proxima chamada
    lastInside = inside;
    res.json({ inside });
});
